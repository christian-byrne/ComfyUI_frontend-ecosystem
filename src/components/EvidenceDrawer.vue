...
<template>
  ...
  >
    ...
    >
      <header class="sticky top-0 bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 px-5 py-4 flex items-start justify-between gap-4">
        <div>
          ...
          <h2 class="font-mono text-sm font-semibold text-zinc-900 dark:text-zinc-100">
            {{ patternId }} <span class="text-zinc-400 dark:text-zinc-500">·</span> {{ pack }}
          </h2>
          <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">{{ evidence.length }} row{{ evidence.length === 1 ? '' : 's' }}</p>
        </div>
        <button
          ref="closeBtnRef"
      ...
      <ol class="divide-y divide-zinc-100 text-sm">
        ...
        >
          ...
          >{{ row.file }}<span v-if="row.lines?.length" class="text-zinc-400 dark:text-zinc-500">:{{ row.lines.join(',') }}</span></a>
          <span v-else class="font-mono text-xs text-zinc-700 dark:text-zinc-300 break-all">
            {{ row.file }}<span v-if="row.lines?.length" class="text-zinc-400 dark:text-zinc-500">:{{ row.lines.join(',') }}</span>
          </span>
          <div v-if="row.variant || row.breakage_class" class="mt-1 flex gap-2 text-[11px]">
            <span v-if="row.variant" class="px-1.5 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300">{{ row.variant }}</span>
            <span v-if="row.breakage_class" class="px-1.5 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300">{{ row.breakage_class }}</span>
          </div>
        </li>
