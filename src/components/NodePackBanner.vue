...
<template>
  <a
    ...
    target="_blank"
    rel="noreferrer"
    class="block rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm transition hover:border-zinc-300 hover:bg-zinc-50"
    data-testid="node-pack-banner"
  >
    <div class="flex items-center justify-between gap-3">
      <span class="truncate font-mono text-xs text-zinc-700">{{ repo }}</span>
      <span
        v-if="entry"
      ...
      <span
        v-else
        class="shrink-0 rounded bg-zinc-50 px-1.5 py-0.5 text-[11px] text-zinc-400"
    ...
    <div
      v-if="!dense && entry"
      class="mt-1 flex items-center gap-3 text-[11px] text-zinc-400"
    >
      <span v-if="entry.forks !== undefined">⑂ {{ entry.forks }}</span>
      <span v-if="entry.last_commit">last commit {{ entry.last_commit }}</span>
      <span v-if="entry.archived" class="text-rose-500">archived</span>
    </div>
  </a>
</template>
