...
const emit = defineEmits<{
  ...
  (e: 'enrichFinished', repo: string): void
}>()
...
watch(
  ...
  (finished) => {
    if (finished) emit('enrichFinished', props.repo)
  },
  { immediate: true }
)

const targetPackId = computed(() => repoToPackId(props.repo) ?? encodeURIComponent(props.repo))
</script>
  ...
  >
    ...
    >
      ...
      <div class="flex gap-1">
        ...
        <dd class="font-mono">
          {{ weightedImpact.toFixed(1) }}
        </dd>
      </div>
      <span v-if="hasError" class="sr-only" data-testid="tile-error" :data-repo="repo">
