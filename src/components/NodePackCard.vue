...
<template>
  <article
    class="flex size-full flex-col overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 transition-colors duration-200 hover:bg-zinc-50 dark:hover:bg-zinc-800 select-none"
  >
    <NodePackBanner v-if="bannerRepo" :repo="bannerRepo" />

    <div class="flex min-h-0 flex-1 flex-col px-3 py-2">
      <h3
        class="truncate overflow-hidden text-xs font-bold text-ellipsis text-zinc-900 dark:text-zinc-100"
      >
        {{ nodePack.name ?? nodePack.id }}
      </h3>
      <p
        v-if="nodePack.description"
        class="my-0 mb-1 line-clamp-3 min-h-12 flex-1 overflow-hidden text-xs/4 font-medium wrap-break-word text-zinc-500 dark:text-zinc-400"
      >
        {{ nodePack.description }}
      </p>
      <div
        v-if="publisherName"
        class="mt-1 flex max-w-40 truncate text-xs/3 font-medium text-zinc-500 dark:text-zinc-400"
      >
        {{ publisherName }}
      </div>
    </div>
    ...
    <div
      class="flex min-h-10 items-center justify-between border-t border-zinc-200 dark:border-zinc-800 px-3 py-2 text-xs font-medium text-zinc-500 dark:text-zinc-400"
    >
      <span
        v-if="formattedStars"
