...
const REAUDIT_REF =
  "research/meta/sourcegraph-mcp-coverage-investigation.md §Re-audit results";
...
export function buildCommentMarkdown(ctx: CommentContext): string {
  ...
  const verdictLine =
    surface.v2Replacement ?? "(see touch-points DB for v2 target)";
  return [
 ...
 */
export async function openInPrAndCopyComment(
  ctx: CommentContext,
): Promise<OpenAndCopyResult> {
