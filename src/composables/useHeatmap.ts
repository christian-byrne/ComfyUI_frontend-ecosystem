...
/** Pattern ids we've already warned about — keep noise to once-per-id. */
const warnedMissingRollup = new Set<string>()
...
export function buildHeatmapMatrix(
  ...
  for (const p of source.patterns) {
    ...
    for (const ev of evidence) {
      ...
      if (!cell) {
        cell = { patternId: p.pattern_id, pack: ev.repo, count: 0, evidence: [] }
        cells.set(key, cell)
      }
      cell.count++
  ...
  const patternsOrdered = source.patterns
    .map((p): RollupEntry => {
      const existing = source.rollupByPatternId[p.pattern_id]
      if (existing) return existing
      if (!warnedMissingRollup.has(p.pattern_id)) {
        warnedMissingRollup.add(p.pattern_id)
        // eslint-disable-next-line no-console
        console.warn(
          `[useHeatmap] no rollup entry for pattern_id="${p.pattern_id}" — synthesizing zero-blast-radius row.`
        )
      }
      return {
...
export function useHeatmap(options: UseHeatmapOptions = {}): {
  ...
  const matrix = computed(() =>
    buildHeatmapMatrix(
      { patterns, evidenceByPatternId, evidenceCountByPack, rollupByPatternId },
      options.topPacks ?? 20
    )
  )
