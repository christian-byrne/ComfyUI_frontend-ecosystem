import type { TopPackSort } from '../useTopPacks'
import { beforeEach, describe, expect, it } from 'vitest'

import { ref } from 'vue'
import { _resetPackCoverageCache } from '../usePackCoverage'
import { buildTopPacks, useTopPacks } from '../useTopPacks'
...
describe('useTopPacks', () => {
  ...
  describe('buildTopPacks', () => {
    ...
    it('sorts by patternHits when sort=patternHits', () => {
      ...
      for (let i = 1; i < packs.length; i++) {
        expect(packs[i - 1].patternHits).toBeGreaterThanOrEqual(packs[i].patternHits)
      }
    })
    ...
    it('sorts by weightedImpact when sort=weightedImpact', () => {
      ...
      for (let i = 1; i < packs.length; i++) {
        expect(packs[i - 1].weightedImpact).toBeGreaterThanOrEqual(packs[i].weightedImpact)
      }
    })
  ...
  describe('useTopPacks composable', () => {
    ...
    it('recomputes when sort changes', () => {
      ...
      const _byStars = [...packs.value]

      sort.value = 'weightedImpact'
      const _byImpact = [...packs.value]
