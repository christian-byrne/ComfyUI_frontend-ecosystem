...
import { readTrioCoverage, type StubState, type StubVariant } from '@/composables/useStubFetcher'
  ...
  <section class="p-6">
    <header class="mb-6">
      ...
      <p class="text-zinc-400 text-sm mt-1">
        {{ cards.length }} categories · click a card for patterns + test stubs.
        Stub coverage indicators (v1 · v2 · migration) reflect cached fetches
        from the upstream PR (1-day TTL).
      </p>
    </header>
    ...
    >
      ...
      >
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            <div class="text-xs font-mono text-zinc-500">{{ card.id }}</div>
            <div class="text-sm font-medium mt-0.5 truncate">{{ card.name }}</div>
          </div>
        ...
        <p class="text-xs text-zinc-500 mt-2 line-clamp-2">{{ card.intent }}</p>
