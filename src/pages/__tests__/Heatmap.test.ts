...
vi.mock("@/data", () => {
  ...
  const rollupByPatternId = Object.fromEntries(
...
describe("Heatmap.vue", () => {
  ...
  it("orders rows by blast_radius desc (P2=9 before P1=5)", async () => {
    ...
    const labels = w
      ...
      .map((c) => c.attributes("data-pattern"));
  ...
  it("sets the highest-count cell to the maximum intensity color", async () => {
    ...
    const top = w
      ...
      .find(
        (c) =>
          c.attributes("data-pattern") === "P1" &&
          c.attributes("data-pack") === "pack-a",
      );
    expect(top).toBeTruthy();
    expect(top!.attributes("style")).toContain(
