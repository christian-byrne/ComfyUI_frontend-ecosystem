...
describe('apiDiff.vue (N16b)', () => {
  function mountPage() {
    ...
    return router.isReady().then(() => mount(RouterView, { global: { plugins: [router] } }))
  }
  ...
  it('renders one card per migration entry with v1/v2 columns and a status badge', async () => {
    ...
    expect(first.find('[data-testid="api-diff-status-badge"]').exists()).toBe(true)
  })
  ...
  it('renders the search, picker, and status-chip controls', async () => {
    ...
    expect(wrapper.find('[data-testid="api-diff-status-chips"]').exists()).toBe(true)
  })
  ...
  it('filters cards by the search query', async () => {
    ...
    await wrapper.find('[data-testid="api-diff-search"]').setValue(target.id)
  ...
  it('shows an empty state when no patterns match', async () => {
    const wrapper = await mountPage()
    await wrapper.find('[data-testid="api-diff-search"]').setValue('definitely-no-such-pattern-zzz')
