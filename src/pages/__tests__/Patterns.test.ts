...
describe('patterns.vue', () => {
  ...
  it('searchPatterns matches across the full dataset, including pack names (DASH-FB-3)', () => {
    ...
    const visiblePack = store.patterns.flatMap((p) => p.evidence).find((e) => e.repo)?.repo
    ...
    for (const p of matches) {
      const hit =
        p.evidence.some((e) => (e.repo ?? '').toLowerCase().includes(slug.toLowerCase())) ||
  ...
  it('clicking a sortable header flips sort order (DASH-FB-5)', async () => {
    ...
    await flushPromises()
    const firstEvDesc = Number(wrapper.findAll('tbody tr')[0].findAll('td')[4].text())

    // Second click on same column → flip to ASC. Top row should now have
    // the smallest evidence count.
    await evidenceHeader.trigger('click')
    await flushPromises()
    const firstEvAsc = Number(wrapper.findAll('tbody tr')[0].findAll('td')[4].text())
