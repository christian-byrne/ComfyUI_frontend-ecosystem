...
const rows = computed<Row[]>(() => {
  ...
  for (const p of matches) {
    ...
    if (packNeedle) {
      const hit = p.evidence.some((e) => (e.repo ?? '').toLowerCase().includes(packNeedle))
      if (!hit) continue
    }
    out.push({
  ...
  <section class="space-y-6">
    ...
    <div class="space-y-4">
      ...
      <div>
        ...
        <div role="group" aria-labelledby="patterns-family-label" class="flex flex-wrap gap-1.5">
          <button
      ...
      <div>
        ...
        <div role="group" aria-labelledby="patterns-behavior-label" class="flex flex-wrap gap-1.5">
          <button
