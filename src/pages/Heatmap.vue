...
import type { HeatmapCell } from '@/composables/useHeatmap'
...
<template>
  <section class="space-y-4">
    <header class="flex flex-wrap items-baseline justify-between gap-4">
      <div>
        <h1 class="text-2xl font-semibold tracking-tight">Pattern × Pack Heatmap</h1>
        <p class="text-sm text-zinc-600 dark:text-zinc-400">
          Rows: {{ matrix.patterns.length }} patterns sorted by blast radius (desc). Columns: top
          {{ matrix.packs.length }} packs by total evidence. Cell intensity: evidence count
      ...
      <div class="flex items-center gap-4">
        ...
        >
          ...
          <span
            ...
            :style="{
              backgroundImage: 'linear-gradient(to right, rgb(250 250 250), rgb(24 24 27))'
            }"
            aria-hidden="true"
          />
